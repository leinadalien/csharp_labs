...
namespace Transport
{
    ...
    {
        public delegate void TransportIformer(string message);
        public event TransportIformer Notify;
        ...
        public void Refuel()
        {
            ...
            {
                Fuel = MaxFuel;
                Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") Successfully refueled");
            }
            else
            {
                Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") is already filled");
            }
        }
        ...
        public void Use()
        {
            if (!Broken)
            {
                if (Fuel > 0)
                {
                    ...
                    Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") covered a distance of " + ((PreFuel - Fuel) / FuelConsumption * 100) + " km.");
                    if (Broken)
                    {
                        Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") just broke.");
                    }
                } else
                {
                    Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") does not have fuel");
                }
            }
            else
            {
                Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") is broken");
            }
        }
        public void Use(double fuel)
        {
            if (!Broken)
            {
                if (fuel <= Fuel)
                {
                    ...
                    Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") covered a distance of " + (fuel / FuelConsumption * 100) + " km.");
                    if (Broken)
                    {
                        Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") just broke.");
                    }
                ...
                {
                    Notify?.Invoke(TransportInfo + "(" + SerialNumber + ") does not have enough fuel");
                }
            }
            else
            {
