...
namespace Transport
{
    ...
    {
        public event TransportIformer MercedesNotify;
        ...
        public override void SetRegistrationNumber(string RegistrationNumber)
        {
            ...
            MercedesNotify?.Invoke(TransportInfo + " get the registration number [" + RegistrationNumber + "]");
            TransportInfo = "Mercedes " + ToString(Model) + "[" + RegistrationNumber + "]";
        }
        ...
        public void Facing()
        {
            ...
            switch (CountOfDesignActions)
            {
                case 1:
                    ...
                    MercedesNotify?.Invoke("'" + ToString(Model) + "' has been printed on the left side of " + TransportInfo);
                    break;
                case 2:
                    DesignInfo += "\n2. Print '" + ToString(Model) + "' on the right side";
                    MercedesNotify?.Invoke("'" + ToString(Model) + "' has been printed on the right side of " + TransportInfo);
                    break;
                case 3:
                    DesignInfo += "\n3. Print 'Mercedes' on the wheels";
                    MercedesNotify?.Invoke("'Mercedes' has been printed on the wheels of " + TransportInfo);
                    break;
                default:
                    MercedesNotify?.Invoke("There is no more print for " + TransportInfo);
                    CountOfDesignActions--;
                    break;
            }
        } 
